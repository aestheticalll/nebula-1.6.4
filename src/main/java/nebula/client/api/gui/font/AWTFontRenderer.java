    ...
    for (int i = 0; i < text.length(); i++)
    {
      ...
      if (c == '\u00a7')
      {
        ...
        switch (colorIndex)
        {
          ...
          {
            if (colorIndex < 16)
            { // 15 = white, anything else is "special"
              boldFont = false;
              italicFont = false;
              strikethrough = false;
              underline = false;
      ...
      if (data != null)
      {
        ...
        if (strikethrough)
        {
          //RenderUtils.line(x, y + data.height / 2.0f, x + data.width - AWTFont.CHAR_OFFSET, y + data.height / 2.0f, 1.5f, 0xFFFFFF);
        }

        if (underline)
        {
          //RenderUtils.line(x + data.width, y + (data.height - AWTFont.CHAR_OFFSET), x, y + (data.height - AWTFont.CHAR_OFFSET), 1.5f, 0xFFFFFF);
        }
