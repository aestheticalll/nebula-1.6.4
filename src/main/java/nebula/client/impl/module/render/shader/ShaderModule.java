package nebula.client.impl.module.render.shader;
...
import nebula.client.impl.event.render.EventRenderEntity;
...
public class ShaderModule extends Module
{
  ...
  private final Listener<EventRender3D> render3D = event ->
  {
    ...
    RenderUtils.espShader.use();

    RenderUtils.espShader.set("texture", 0);
    RenderUtils.espShader.set("texelSize", 1.0f / r.getScaledWidth(), 1.0f / r.getScaledHeight());

    Color c = ColorUtils.pulse(HUDModule.primary.value(), HUDModule.secondary.value(), 10, 1);
    RenderUtils.espShader.set("color", c.getRed() / 255.0f, c.getGreen() / 255.0f, c.getBlue() / 255.0f, 1.0f);
    RenderUtils.espShader.set("radius", 1.0f);
    RenderUtils.espShader.set("opacity", 0.0f);
